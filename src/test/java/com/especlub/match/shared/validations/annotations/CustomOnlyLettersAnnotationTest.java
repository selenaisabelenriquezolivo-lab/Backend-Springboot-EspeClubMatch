...
class CustomOnlyLettersAnnotationTest {
    @Test
    void testAnnotationTargets() {
        Target target = CustomOnlyLetters.class.getAnnotation(Target.class);
    ...
    void testAnnotationRetention() {
        Retention retention = CustomOnlyLetters.class.getAnnotation(Retention.class);
    ...
    void testAnnotationDocumented() {
        assertTrue(CustomOnlyLetters.class.isAnnotationPresent(Documented.class), "Debe estar documentada");
    }

    @Test
    void testConstraintValidatorClass() {
        Constraint constraint = CustomOnlyLetters.class.getAnnotation(Constraint.class);
        assertNotNull(constraint, "Debe tener la anotación @Constraint");
        assertEquals(CustomOnlyLettersValidator.class, constraint.validatedBy()[0], "El validador debe ser CustomOnlyLettersValidator");
    }

    @Test
    void testDefaultMessage() throws NoSuchMethodException {
        Method messageMethod = CustomOnlyLetters.class.getMethod("message");
        assertEquals("El valor solo puede contener letras (mayúsculas, minúsculas, con tildes) y espacios", messageMethod.getDefaultValue(), "El mensaje por defecto debe ser el esperado");
    }
}
