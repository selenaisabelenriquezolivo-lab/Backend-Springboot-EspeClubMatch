...
class CustomPasswordSecureAnnotationTest {
    @Test
    void testAnnotationTargets() {
        Target target = CustomPasswordSecure.class.getAnnotation(Target.class);
    ...
    void testAnnotationRetention() {
        Retention retention = CustomPasswordSecure.class.getAnnotation(Retention.class);
    ...
    void testAnnotationDocumented() {
        assertTrue(CustomPasswordSecure.class.isAnnotationPresent(Documented.class), "Debe estar documentada");
    }

    @Test
    void testConstraintValidatorClass() {
        Constraint constraint = CustomPasswordSecure.class.getAnnotation(Constraint.class);
        assertNotNull(constraint, "Debe tener la anotación @Constraint");
        assertEquals(CustomPasswordSecureValidator.class, constraint.validatedBy()[0], "El validador debe ser CustomPasswordSecureValidator");
    }

    @Test
    void testDefaultMessage() throws NoSuchMethodException {
        Method messageMethod = CustomPasswordSecure.class.getMethod("message");
        assertEquals("La contraseña debe tener al menos 8 caracteres, una mayúscula, una minúscula, un número y un carácter especial.", messageMethod.getDefaultValue(), "El mensaje por defecto debe ser el esperado");
    }
}
